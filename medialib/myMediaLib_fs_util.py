...
_EXT_CALL_FREQ = 10
def _extention_point_func(*args):
	pass	

def _collect_media_files_in_folder_list(folderL,*args):
	music_folderL = []
	#if self:
...
def find_new_music_folder_simple(init_dirL,*args):
	...
	folders_list = tuple(_bulld_subfolders_list(dirL,'verbose'))	
	t = time.time()
	print()
	...
	t = time.time()
	music_folderL = _collect_media_files_in_folder_list(joined_folder_list,'verbose')
	print()
...
def find_new_music_folder(init_dirL, prev_folderL, DB_folderL,*args):
	...
	f_l= tuple(_bulld_subfolders_list(init_dirL))			
	print()
	time_stop_diff = time.time()-t
	...
	music_folderL = _collect_media_files_in_folder_list(new_folderL)
	
	# check if initial folder root folder itself containes media
	if not music_folderL:
		music_folderL = _collect_media_files_in_folder_list(init_dirL)
