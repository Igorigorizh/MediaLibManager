...
def get_mpd_extend_status(mpd_client, host, socket, mpd_path_prefix, media_lib_allcrc32L):
    ...
    return {'list_length': list_length,
            ...
            'rest_sec': (
                                self.__winampObj.getPlayingTrackLength() * 1000 - self.__winampObj.getPlayingTrackPosition()) / 1000,
            'duration_sec': (self.__winampObj.getPlayingTrackLength())}
...
def load_mpd_playlist_via_tag_num(db, mpd_client, host, socket, mpd_path_preffix, object_key, mode):
    ...
    if mode.lower() == "track_tag":
        ...
        # ���� ��� ������� ������, �� � ���� ����� �� ���� ��������� �������� �� ���� ������� �� ���� ��������.
        artist_rel_albumD = \
            getArtist_Album_relationD_and_simpleMetaD_viaCRC32L(None, db, [artistCRC32], [], 'with_relation')[
                'artist_rel_albumD']
    ...
    if len(resD['errorLog']) > 0:
        logger.critical('Errors after load_mpd_playlist_via_metaD [%i]' % (len(resD['errorLog'])))
        print('errors at loading:', str(len(resD['errorLog'])))
        for a in resD['errorLog'][:10]:
            logger.debug('erorrs from errorLog [%s]' % (str(a)))
...
def navigate_check_all_cue_image_albums(db, mpdHandle, mpd_host, mpd_socket, mpd_path_prefix, saved_dump, *args):
    ...
    for album_crc32 in cue_album_crc32L:
        ...
        ts = time.time()
        try:
            r = load_mpd_playlist_via_tag_num(db, mpdHandle, mpd_host, mpd_socket, mpd_path_prefix, album_crc32,
                                              "album")
        except Exception as e:
            print(e)
        ...
        if len(r['sorted_crc32L']) != len(l):
            ...
            if len(r['sorted_crc32L']) != (len(r['sorted_crc32L']) - len(l)):
                print("Issue prtl:", album_crc32, '[load/skipped', len(r['sorted_crc32L']),
                      (len(r['sorted_crc32L']) - len(l)), ']')
                if album_crc32 in resDAll['albumD']:
                    resDAll['albumD'][album_crc32]['file']
            else:
                print('Issue full:', len(album_issueL), '[', len(r['sorted_crc32L']),
                      (len(r['sorted_crc32L']) - len(l)), ']')
                if album_crc32 in resDAll['albumD']:
                    resDAll['albumD'][album_crc32]['file']
        if i % mode_dif == 0:
            ...
            resD = getAlbumD_fromDB(None, db, None, [a[0] for a in album_issueL])
            d = pickle.dump({'reslist': {"album_issueL": album_issueL, 'albumD': resD['albumD']}, 'checkedL': checkedL},
                            f)
            f.close()
        i += 1
    ...
    while uri_folder != '' and uri_folder != uri_check_node_folder and not isFolderInMpdDb:
        ...
        try:
            ...
            if 'No such directory' in str(e):
                ...
                logger.warning(
                    'Warning: in check_update_mpd_db uri_folder find update to parent [%s]' % (str(uri_folder)))
                continue
            else:
                logger.critical('MPD  Error: in check_update_mpd_db find[%s]' % (str(e)))
        print(2)
